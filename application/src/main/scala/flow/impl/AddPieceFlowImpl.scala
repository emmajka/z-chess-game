...
case class AddPieceFlowImpl(gameRepository: GameRepository) extends AddPieceFlow {
  override def run(gameId: String, pieceType: PieceType, targetPosition: Position): Task[Int] =
    for
      ...
      pieceWithSamePosition = allActivePieces.find(gp => gp.xCoordinate == targetPosition.x && gp.yCoordinate == targetPosition.y && gp.active)
      ...
      newPieceId = allActivePieces.maxByOption(_.pieceId).map(_.pieceId + 1).getOrElse(1)
